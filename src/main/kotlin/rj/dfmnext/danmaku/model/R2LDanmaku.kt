...
open class R2LDanmaku(duration: Duration) : BaseDanmaku() {
    ...
    companion object {
        protected const val MAX_RENDERING_TIME = 100L
        protected const val CORDON_RENDERING_TIME = 40L
    }
    ...
    override fun layout(displayer: IDisplayer, x: Float, y: Float) {
        if (mTimer != null) {
            ...
            val deltaDuration = currMS - time
            if (deltaDuration > 0 && deltaDuration < (duration?.value ?: 0)) {
    ...
    override fun measure(displayer: IDisplayer, fromWorkerThread: Boolean) {
        ...
        mStepX = mDistance / (duration?.value?.toFloat() ?: 1f)
    }
}
